...
public class View extends Model<View>
{
     ...
     * @param dataType
     * @return Column 
     * return the first Column with the dataType specified.
     * Use for research the 'location' column
     */
	public static Column getColByDataType(View v,String dataType){
		List<Column> cl=v.getColumns();
		for(Column m : cl){
			if(m.getDataTypeName().equals(dataType)){
				return m;
			}
		}
		return null;
     ...
     * @return column
     */
    public Column getColumnByIdentifier(String name)
    {
        for (Column column : this.columns)
        {
...
//inizio test1
    public List<Row> getRows(Response r) throws RequestException
    {
        
        return results(r, Row.class);
    }
//fine test1
    ...
    public static View find(String id, Connection conn) throws RequestException, IOException
    {
     ...
     */
    public View copy(Connection conn) throws RequestException, IOException
    {
        MultivaluedMap<String, String> params = new MultivaluedMapImpl();
        params.putSingle("viewId", getId());
        params.putSingle("method", "copy");
	...
	@SuppressLint("NewApi")
	public List<Row> parseXml(String xmlUrl,View meta){
    	Row row=new Row();
        Document doc;
        List<View.Row> listrow=new ArrayList<View.Row>();
        try {
                                       
                doc=DocumentBuilderFactory.newInstance().newDocumentBuilder().parse(new InputSource(new ByteArrayInputStream(xmlUrl.getBytes("utf-8"))));
                ...
                for(int i=0;i<notes.getLength();i++){//per ogni
                        ...
                        if(c.getNodeType()==Node.ELEMENT_NODE){//controlliamo se questo è un nodo elemento (un tag)
                                ...
                                NodeList noteDetails=c.getChildNodes();  //per ogni row abbiamo i vari dettagli
                                for(int j=0;j<noteDetails.getLength();j++){
                                        Node c1=noteDetails.item(j);
                                                ...
                                                if(c1.getNodeType()==Node.ELEMENT_NODE){ //anche in questo caso controlliamo se si tratta di tag
                                                    NodeList rowDetails=c1.getChildNodes();
                                                    
                                                  for(int k=0;k<rowDetails.getLength();k++){
                                                    	Node c2=rowDetails.item(k);
                                                    ...
                                                    if(c2.getNodeType()==Node.ELEMENT_NODE){
                                                    	...
                                                    	//id.item(1).getNodeName();
                                                        String nodeName=list.item(t).getNodeName(); //leggo il nome del tag
                                                        String nodeValue=list.item(t).getTextContent();//leggo il testo in esso contenuto
                                                        if(nodeName.equals("location")){
                                                        	LinkedHashMap<String, Object> map=new LinkedHashMap<String, Object>();
                                                        	NamedNodeMap maps=list.item(t).getAttributes();
                                                        	map.put("latitude", maps.getNamedItem("latitude").getTextContent());
                                                        	Log.d("parsexml",maps.getNamedItem("latitude").getTextContent());
                                                        	map.put("longitude", maps.getNamedItem("longitude").getTextContent());
                                                        	row.putDataField(View.getColByDataType(meta,"location"), map);
                                                        	
                                                        }
                                                        else
                                                        row.putDataField(meta.getColumnByIdentifier(nodeName.toUpperCase()), nodeValue);
                                                        
                                                       //Log.d("parsexml",nodeName);
                                                       //Log.d("parsexml",nodeValue);
